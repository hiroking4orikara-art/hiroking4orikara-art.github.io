...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_32_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'basic_act_for_environmental_pollution_control',
    'basic_environment_act',
    'ministry_of_the_environment',
    'three_rs',
    'sound_material_cycle_society',
    'tax_cut',
    'public_bond',
    'national_bond',
    'local_bond',
    'fiscal_investment_and_loan_program'
];
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 310 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_32_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
