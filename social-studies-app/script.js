const app = {
    ...
    renderUnits(units) {
        ...
        units.forEach(unit => {
            ...
            if (unit.id && unit.id.startsWith('c_')) {
                // 公民の表示レイアウト
                let textbookListHtml = '';
                if (window.TEXTBOOK_MAP && window.TEXTBOOK_MAP[unit.id] && Array.isArray(window.TEXTBOOK_MAP[unit.id])) {
                    window.TEXTBOOK_MAP[unit.id].forEach(item => {
                        const url = `https://hiroking4orikara-art.github.io/social-studies-web-textbook/${item.file}`;
                        textbookListHtml += `
                            <div class="textbook-row" style="display: flex; align-items: center; margin-top: 8px; gap: 10px; width: 100%;">
                                <button class="unit-btn btn-material" onclick="app.openMaterial('${url}')" style="margin: 0; padding: 6px 12px; font-size: 0.9rem; flex-shrink: 0; background-color: #2ecc71; color: white;">
                                    <i class="fas fa-book-open"></i> 教科書を見る
                                </button>
                                <span style="font-size: 0.95rem; color: #555; text-align: left;">${item.title}</span>
                            </div>
                        `;
                    });
                }
                ...
                el.innerHTML = `
                    <div class="unit-header" style="border-bottom: 1px solid #eee; padding-bottom: 8px; margin-bottom: 8px; width: 100%;">
                        <span class="unit-title" style="font-size: 1.2rem; font-weight: bold;">${unit.title} <span class="unit-count">(全${c}問)</span></span>
                    </div>
                    <div class="unit-controls" style="display: flex; gap: 10px; justify-content: flex-end; width: 100%; margin-bottom: 12px;">
                        ${videoButtonsHtml}
                        <button class="unit-btn btn-quiz" onclick="app.startQuiz('${unit.id}')" style="margin: 0;">
                            <i class="fas fa-pencil-alt"></i> クイズ
                        </button>
                    </div>
                    <div class="textbook-section" style="display: flex; flex-direction: column; width: 100%; border-top: 1px dashed #ddd; padding-top: 8px; align-items: flex-start;">
                        ${textbookListHtml}
                    </div>
                `;
            } else {
                // 地理・歴史の表示レイアウト（「動画」ボタンのすぐ左に「教科書」ボタンを設置）
                let textbookBtnHtml = '';
                if (window.TEXTBOOK_MAP && window.TEXTBOOK_MAP[unit.id] && typeof window.TEXTBOOK_MAP[unit.id] === 'string') {
                    const url = `https://hiroking4orikara-art.github.io/social-studies-web-textbook/${window.TEXTBOOK_MAP[unit.id]}`;
                    textbookBtnHtml = `
                        <button class="unit-btn btn-material" onclick="app.openMaterial('${url}')" style="background-color: #2ecc71; color: white; margin-right: 4px;">
                            <i class="fas fa-book-open"></i> 教科書
                        </button>
                    `;
                }
