...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_15_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'ministry_of_justice',
    'ministry_internal_affairs',
    'ministry_economy',
    'ministry_land_infra',
    'ministry_education',
    'ministry_foreign_affairs',
    'ministry_finance',
    'ministry_health_labor',
    'civil_servants',
    'bureaucrats'
];
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 140 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_15_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
