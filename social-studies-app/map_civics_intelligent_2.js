...
try {
    ...
    const keywordMap = {
        ...
        "rights": ["権利"],
        "duties": ["義務", "責任"],
        "fundamental_human_rights": ["基本的人権"],
        ...
        "non_intervention": ["内政不干渉の原則"],
        "sovereign_equality": ["主権平等の原則"],
        ...
        "g7_summit": ["主要国首脳会議", "サミット"],
        "oda": ["政府開発援助", "ODA"],
        "jica": ["国際協力機構", "JICA"],
        "jocv": ["青年海外協力隊"],
        "un_general_assembly": ["総会"],
    ...
    // See which questions are STILL missing (re-evaluate instead of match)
    // To avoid Function evaluation issues due to unescaped quotes from the previous replace
    // I'll just use the old missing list and see what hasn't been mapped.
    const mappings = [];
    let replaced = 0;
    const results = [];

    // Filter out images that ALREADY exist in quizDataStr
    const unusedImages = civicsImages.filter(img => !quizDataStr.includes(img.filename));

    missingCivics.forEach(q => {
        // Skip if this question appears to have an image now
        const qEscapedCheck = q.q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); 
        const blockRegexCheck = new RegExp(`(q:\\s*"${qEscapedCheck}"[\\s\\S]*?)(a:\\s*".*?")([\\s\\S]*?\\n*\\s*\\})`, 'g');
        const matchCheck = blockRegexCheck.exec(quizDataStr);
        if (matchCheck && matchCheck[3].includes('answerImg')) {
        ...
        for (const img of unusedImages) {
            let score = 0;
            const ansParams = q.a.toLowerCase();
            const qStr = q.q.toLowerCase();
            
            // Check dictionary using answer OR question text
            for (const [kw, answers] of Object.entries(keywordMap)) {
                if (answers.some(a => q.a.includes(a) || q.q.includes(a) || (q.comment && q.comment.includes(a)))) {
            ...
            if (qStr.includes(img.keyword.replace(/_/g, ' '))) score += 5;
        ...
        if (bestMatch && highestScore > 0) {
            ...
            results.push(`MATCHED: ${q.a} -> ${bestMatch.filename}`);
            // remove from unused
            unusedImages.splice(unusedImages.indexOf(bestMatch), 1);
        }
    });

    if (replaced > 0) {
        fs.writeFileSync('quiz_data.js', quizDataStr, 'utf8');
        fs.writeFileSync('civics_mapping_results_2.txt', results.join('\n'));
        console.log(`Successfully mapped and applied ${replaced} MORE civics images!`);
    } else {
