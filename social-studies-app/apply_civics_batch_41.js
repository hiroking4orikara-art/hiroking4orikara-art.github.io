...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_41_') && f.endsWith('.png'));
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 400 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    // Note: Since the first two images might have been generated earlier (different timestamp), the regex handles any timestamp.
    const file = files.find(f => f.match(new RegExp(`^g_civics_41_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
