...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_46_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'street_children',
    'hunger_map',
    'convention_on_rights_of_child',
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 450 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_46_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
