...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_13_') && f.endsWith('.png'));
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 120 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    // We use a strict match to prevent substring issues like in Batch 12
    const file = files.find(f => f.match(new RegExp(`^g_civics_13_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
