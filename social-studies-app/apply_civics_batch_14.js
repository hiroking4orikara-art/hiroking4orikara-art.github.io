...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_14_') && f.endsWith('.png'));
...
const orderedNames = [
    ...
    'supreme_court',
    'lower_courts',
    'no_confidence_resolution',
    'dissolution_house_reps',
    'cabinet_resignation',
    'ministries_agencies',
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 130 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_14_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
