...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_29_') && f.endsWith('.png'));
...
const orderedNames = [
    ...
    'cashless_payment',
    'monetary_policy',
    'managed_currency_system',
    'unemployed_recession'
];
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 280 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_29_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
