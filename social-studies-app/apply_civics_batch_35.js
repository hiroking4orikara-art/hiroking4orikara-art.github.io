...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_35_') && f.endsWith('.png'));
...
const orderedNames = [
    ...
    'basic_act_for_establishing_a_sound_material_cycle_society',
    'national_park',
    'eco_mark',
    'zero_emission',
    'environmental_tax'
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 340 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_35_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
