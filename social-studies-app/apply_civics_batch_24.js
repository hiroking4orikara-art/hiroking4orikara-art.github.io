...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_24_') && f.endsWith('.png'));
...
const orderedNames = [
    ...
    'pos_system',
    'consumer_movement',
    'consumer_affairs_center',
    'regular_employee',
    'non_regular_employee',
    'neet',
    'unemployed',
    'equal_employment_opportunity_act'
];
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 230 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_24_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
