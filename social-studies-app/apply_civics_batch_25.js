...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_25_') && f.endsWith('.png'));
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 240 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_25_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
