...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_40_') && f.endsWith('.png'));
...
const orderedNames = [
    ...
    'ie_system',
    'different_surnames',
    'children_on_waiting_lists',
    'aging_society_with_fewer_children',
    'social_group'
];
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 390 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_40_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
