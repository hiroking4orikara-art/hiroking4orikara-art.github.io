...
for (const [section, questions] of Object.entries(parsedData)) {
    if (!civicsSections.includes(section)) {
        cleanData[section] = questions.filter(q => {
            const isFromBatch = allCivics.some(civ => getSanitized(civ.q) === getSanitized(q.q) && getSanitized(civ.a) === getSanitized(q.a));
            let isFromBackupCivics = false;
            civicsSections.forEach(cs => {
               if (parsedBackup1[cs] && parsedBackup1[cs].some(civ => getSanitized(civ.q) === getSanitized(q.q) && getSanitized(civ.a) === getSanitized(q.a))) {
...
// Calculate what's already mapped
const existingCivicsQs = new Set();
civicsSections.forEach(cs => {
    cleanData[cs].forEach(q => {
        if(q.q && q.a) {
             existingCivicsQs.add(getSanitized(q.q) + '|' + getSanitized(q.a));
        }
    });
});

console.log('Initially restored Civics:', existingCivicsQs.size);

// Find batches that aren't in there yet
const missingFromBatches = allCivics.filter(civ => {
    return !existingCivicsQs.has(getSanitized(civ.q) + '|' + getSanitized(civ.a));
});

console.log('Unique missing from batches:', missingFromBatches.length);
...
console.log(`Final Restored Civics: ${totalC} questions (added ${missingFromBatches.length} missing from batches to cw_1)`);
