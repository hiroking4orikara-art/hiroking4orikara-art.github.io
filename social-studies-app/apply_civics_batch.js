...
try {
    ...
    if (!batchNum || isNaN(batchNum)) {
        console.error("Please provide a batch number (1-indexed). Example: node apply_civics_batch.js 3");
        process.exit(1);
    }
    
    const inputFile = `civics_batch_${batchNum}.json`;
    ...
    batch.forEach(item => {
        ...
        if (timestamp) {
            matchedFile = files.find(f => f.includes(`_${timestamp}_`) || f.includes(`_${timestamp}.`));
        }
        if (!matchedFile) {
    ...
    if (replaced > 0) {
        ...
        console.log(`Successfully applied ${replaced} new images to quiz_data.js! (Batch ${batchNum})`);
    } else {
