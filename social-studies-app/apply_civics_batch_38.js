...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_38_') && f.endsWith('.png'));
...
const orderedNames = [
    ...
    'international_division_of_labor',
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 370 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_38_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
