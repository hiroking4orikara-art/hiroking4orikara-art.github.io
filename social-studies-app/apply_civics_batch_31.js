...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_31_') && f.endsWith('.png'));
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 300 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_31_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
