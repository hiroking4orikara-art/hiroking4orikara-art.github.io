...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_23_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'multi_level_marketing',
    'product_liability_act',
    'consumer_contract_act',
    'basic_act_on_consumer_policies',
    'consumers_four_rights',
    'consumer_affairs_agency',
    'cooling_off',
    'distribution',
    'rationalization_of_distribution',
    'wholesale_trade'
];
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 220 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_23_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
