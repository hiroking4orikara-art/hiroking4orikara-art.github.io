...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_39_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'basic_act_declining_birthrate',
    'elderly_care_by_elderly',
    'dynamic_engagement_all_citizens',
    'science',
    'traditional_culture',
    'annual_events',
    'cultural_properties',
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 380 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_39_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
