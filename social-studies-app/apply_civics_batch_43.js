...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_43_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'fuel_cell_vehicle',
    'cop3_kyoto_conference',
    'kyoto_protocol',
    'global_environmental_issues',
    'un_conference_human_environment',
    'unep',
    'earth_summit',
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 420 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_43_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
