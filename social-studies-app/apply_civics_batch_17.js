...
const files = fs.readdirSync(srcDir).filter(f => f.startsWith('g_civics_17_') && f.endsWith('.png'));
console.log(`Found ${files.length} images to apply.`);

const orderedNames = [
    'independence_judiciary',
    'judicial_review',
    'civil_trial',
    'saibanin_system',
    'prosecution_review_commission',
...
for (let i = 0; i < 10; i++) {
    const questionIndex = 160 + i;
    const namePrefix = orderedNames[i];
    
    // Find matching image file
    const file = files.find(f => f.match(new RegExp(`^g_civics_17_${namePrefix}_\\d+\\.png$`)));
    if (!file) {
